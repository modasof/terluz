...
public static function obtenerPagina(){ 
	try {		
		...
		$select=$db->query("SELECT * FROM testimonios");
...
public static function obtenerPaginaPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM testimonios WHERE id='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Testimonios('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM testimonios WHERE id='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function actualizarTestimonio($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE testimonios SET 
								nombres=:nombres,
								oficio=:oficio,
		...
		$update->bindValue('oficio',$oficio);
		$update->bindValue('testimonio',$testimonio);
