...
public static function obtenerListacuentas(){
	try {
    	...
    	$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
/*******************************************************
** FUNCION PARA MOSTRAR LA LISTA DE FOLDERS	  **
********************************************************/
public static function obtenerListafolder($categoria){
	try {
		$db=Db::getConnect();

		$select=$db->query("SELECT * FROM categorias_folder WHERE estado_folder='1' and categoria_ppal='".$categoria."' order by nombre_folder asc");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
public static function obtenerNodisponibles($cuenta){
	try {
		...
		$select=$db->query("SELECT * FROM gestion_documental WHERE imagen='No-Aplica' and cuenta_id_cuenta='".$cuenta."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
********************************************************/
public static function obtenerPaginavariosporfolder($cuenta,$folder){
	try {
		$db=Db::getConnect();

		$select=$db->query("SELECT * FROM documentos_varios WHERE cuenta_id_cuenta='".$cuenta."' and midocumento_publicado='1' and id_folder='".$folder."' ");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginavarios($cuenta){
	try {
		...
		$select=$db->query("SELECT * FROM documentos_varios WHERE cuenta_id_cuenta='".$cuenta."' and midocumento_publicado='1' ");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
** FUNCION PARA MOSTRAR EL NOMBRE DEl EQUIPO**
********************************************************/
public static function ObtenerNombreFolder($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT nombre_folder FROM categorias_folder WHERE id='".$id."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
			$lacaja = $ncaja['nombre_folder'];
		}
		return $lacaja;
	}
	catch(PDOException $e) {
...
public static function obtenerPaginaPor($id){
	...
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM gestion_documental WHERE id_registro='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginavariosPor($id){
	...
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM documentos_varios WHERE id_midocumento='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM gestion_documental WHERE id_registro='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function eliminarvariosPor($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("UPDATE documentos_varios set midocumento_publicado='0' WHERE id_midocumento='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function desactivarDocumento($cuenta,$modulo,$documento,$imagen){
	try {
		$db=Db::getConnect();
		$consulta="INSERT INTO gestion_documental (cuenta_id_cuenta,modulo_id_modulo,documento_id_documento,imagen) VALUES ('".$cuenta."','".$modulo."','".$documento."','".$imagen."')";
...
public static function actualizar($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE gestion_documental SET
								imagen=:imagen,
...
public static function actualizarvarios($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE documentos_varios SET
								cuenta_id_cuenta=:cuenta_id_cuenta,
								id_folder=:id_folder,
		...
		$update->bindValue('imagen',utf8_decode($imagen));
		$update->bindValue('cuenta_id_cuenta',utf8_decode($cuenta_id_cuenta));
		$update->bindValue('id_folder',utf8_decode($id_folder));
...
public static function guardar($campos,$imagen){
	...
	try {
		...
		$db=Db::getConnect();
				$consulta="DELETE from  gestion_documental where modulo_id_modulo='".$modulo_id_modulo."' and documento_id_documento='".$documento_id_documento."' and cuenta_id_cuenta='".$cuenta_id_cuenta."'";
				$select=$db->query($consulta);

		$insert=$db->prepare('INSERT INTO gestion_documental VALUES (NULL,:imagen,:documento_id_documento,:modulo_id_modulo,:cuenta_id_cuenta,:alerta,:fecha_expiracion,:marca_temporal,:gestion_estado,:gestion_publicado,:creado_por)');
...
INSERT INTO documentos_varios(id_midocumento, cuenta_id_cuenta, nombre_documento, imagen, fecha_expira, alerta, marca_temporal, creado_por, estado_midocumento, midocumento_publicado)
***************************************************************/
public static function guardarvarios($campos,$imagen){
	...
	try {
		...
		$insert=$db->prepare('INSERT INTO documentos_varios VALUES (NULL,:cuenta_id_cuenta,:id_folder, :nombre_documento, :imagen, :fecha_expira, :alerta, :marca_temporal, :creado_por, :estado_midocumento, :midocumento_publicado)');

		$t = strtotime($fecha_expira);
		$nuevafecha=date('y-m-d',$t);

		$insert->bindValue('cuenta_id_cuenta',utf8_decode($cuenta_id_cuenta));
		$insert->bindValue('id_folder',utf8_decode($id_folder));
...
*** FUNCION PARA GUARDAR **
***************************************************************/
public static function carpetaextra($campos){
	try {
		$db=Db::getConnect();
		$campostraidos = $campos->getCampos();
		extract($campostraidos);
...
public static function DocumentoExiste($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT imagen FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoRegistro($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT id_registro FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoFechaExpira($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT fecha_expiracion FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoAlerta($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT alerta FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoMarcatemporal($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT marca_temporal FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoEstado($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$consulta="SELECT gestion_estado FROM gestion_documental WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'";
		$select=$db->query($consulta);
		//echo($consulta);
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumental('',$camposs);
	...
	try {
		...
		foreach($cajas as $ncaja){
			...
			$marca_temporal="0000-00-00";
				$db=Db::getConnect();
				$consulta="INSERT INTO gestion_documental(documento_id_documento,modulo_id_modulo,cuenta_id_cuenta,gestion_estado,fecha_expiracion,marca_temporal) VALUES ('".$eldocumento."','".$id_modulo."','".$cuenta."','".$gestion_estado."','".$fecha_expiracion."','".$marca_temporal."')";
				$select=$db->query($consulta);
...
public static function obtenerUltimosDocumentos(){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM gestion_documental WHERE imagen<>'No-Aplica' order by id_registro desc limit 10");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumental('',$camposs);
		return $campos;
	}
