...
public static function obtenerPagina(){ 
	try {		
		...
		$select=$db->query("SELECT * FROM ventajas");
    	$camposs=$select->fetchAll();
    	$campos = new Ventajas('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginaPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM ventajas WHERE id='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Ventajas('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM ventajas WHERE id='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function actualizar($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE ventajas SET 
								imagen=:imagen,
								titulo=:titulo,
								texto=:texto
								WHERE id=:id');
...
public static function guardar($campos,$imagen){
	try {
		...
		$insert=$db->prepare('INSERT INTO ventajas VALUES (NULL,:imagen,:titulo,:texto)');
