...
public static function obtenerPagina(){ 
	try {		
		...
		$select=$db->query("SELECT * FROM terminos");
    	$camposs=$select->fetchAll();
    	$campos = new Terminos('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginaPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM terminos WHERE id='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Terminos('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM terminos WHERE id='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function actualizar($id,$campos){
	try {
		...
		$update=$db->prepare('UPDATE terminos SET 
								titulo=:titulo,
								texto=:texto
								WHERE id=:id');
