...
public static function obtenerListaequipos(){
	try {
		...
		$select=$db->query("SELECT * FROM proveedores WHERE estado_proveedor
='1' ");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
...
public static function obtenerNodisponibles($cuenta){
	try {
		...
		$select=$db->query("SELECT * FROM gestion_documentalprov WHERE imagen='No-Aplica' and cuenta_id_cuenta='".$cuenta."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginavarios($cuenta){
	try {
		...
		$select=$db->query("SELECT * FROM documentos_variosprov WHERE cuenta_id_cuenta='".$cuenta."' and midocumento_publicado='1' ");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
...
public static function obtenerEquipos(){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM proveedores WHERE estado_proveedor='1' order by nombre_proveedor asc");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$marcas = $campos->getCampos();
		return $marcas;
...
public static function obtenerPaginaPor($id){
	...
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM gestion_documentalprov WHERE id_registro='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
...
public static function obtenerPaginavariosPor($id){
	...
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM documentos_variosprov WHERE id_midocumento='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM gestion_documentalprov WHERE id_registro='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function eliminarvariosPor($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("UPDATE documentos_variosprov set midocumento_publicado='0' WHERE id_midocumento='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function desactivarDocumento($cuenta,$modulo,$documento,$imagen){
	try {
		$db=Db::getConnect();
		$consulta="INSERT INTO gestion_documentalprov (cuenta_id_cuenta,modulo_id_modulo,documento_id_documento,imagen) VALUES ('".$cuenta."','".$modulo."','".$documento."','".$imagen."')";
...
public static function actualizar($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE gestion_documentalprov SET
								imagen=:imagen,
...
public static function actualizarvarios($id,$campos,$imagen){
	try {
		...
		$update=$db->prepare('UPDATE documentos_variosprov SET
								cuenta_id_cuenta=:cuenta_id_cuenta,
...
public static function guardar($campos,$imagen){
	...
	try {
		...
		$db=Db::getConnect();
				$consulta="DELETE from  gestion_documentalprov where modulo_id_modulo='".$modulo_id_modulo."' and documento_id_documento='".$documento_id_documento."' and cuenta_id_cuenta='".$cuenta_id_cuenta."'";
				$select=$db->query($consulta);

		$insert=$db->prepare('INSERT INTO gestion_documentalprov VALUES (NULL,:imagen,:documento_id_documento,:modulo_id_modulo,:cuenta_id_cuenta,:alerta,:fecha_expiracion,:marca_temporal,:gestion_estado,:gestion_publicado,:creado_por)');
...
public static function guardarvarios($campos,$imagen){
	...
	try {
		...
		$insert=$db->prepare('INSERT INTO documentos_variosprov VALUES (NULL,:cuenta_id_cuenta, :nombre_documento, :imagen, :fecha_expira, :alerta, :marca_temporal, :creado_por, :estado_midocumento, :midocumento_publicado)');
...
public static function DocumentoExiste($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT imagen FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoRegistro($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT id_registro FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoFechaExpira($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT fecha_expiracion FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoAlerta($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT alerta FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoMarcatemporal($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT marca_temporal FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'");
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
    	$cajas = $campos->getCampos();
		foreach($cajas as $ncaja){
...
public static function DocumentoEstado($id,$cuenta,$modulo){
	try {
		$db=Db::getConnect();
		$consulta="SELECT gestion_estado FROM gestion_documentalprov WHERE documento_id_documento='".$id."' and cuenta_id_cuenta='".$cuenta."' and modulo_id_modulo='".$modulo."'";
		$select=$db->query($consulta);
		//echo($consulta);
    	$camposs=$select->fetchAll();
    	$campos = new Gestiondocumentalprov('',$camposs);
	...
	try {
		...
		foreach($cajas as $ncaja){
			...
			$marca_temporal="0000-00-00";
				$db=Db::getConnect();
				$consulta="INSERT INTO gestion_documentalprov(documento_id_documento,modulo_id_modulo,cuenta_id_cuenta,gestion_estado,fecha_expiracion,marca_temporal) VALUES ('".$eldocumento."','".$id_modulo."','".$cuenta."','".$gestion_estado."','".$fecha_expiracion."','".$marca_temporal."')";
				$select=$db->query($consulta);
...
public static function obtenerUltimosDocumentos(){
	try {
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM gestion_documentalprov WHERE imagen<>'No-Aplica' order by id_registro desc limit 10");
		$camposs=$select->fetchAll();
		$campos = new Gestiondocumentalprov('',$camposs);
		return $campos;
	}
