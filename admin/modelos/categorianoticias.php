...
public static function obtenerPagina(){
	try {
    	...
    	$campos = new Categorianoticias('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){
	try {
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM categorianoticias WHERE id='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
public static function actualizar($id,$campos){
	try {
		...
		$update=$db->prepare('UPDATE categorianoticias SET
								categoria=:categoria
								WHERE id=:id');
...
public static function guardar($campos){
	try {
		...
		$insert=$db->prepare('INSERT INTO categorianoticias VALUES (NULL,:categoria)');
