...
public static function obtenerPagina(){ 
	try {		
		...
		$select=$db->query("SELECT * FROM nosotros");
...
public static function obtenerPaginaPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("SELECT * FROM nosotros WHERE id='".$id."'");
		$camposs=$select->fetchAll();
		$campos = new Nosotros('',$camposs);
		return $campos;
	}
...
public static function eliminarPor($id){ 
	try {		
		$db=Db::getConnect();
		$select=$db->query("DELETE FROM nosotros WHERE id='".$id."'");
		if ($select){
			return true;
			}else{return false;}
	}
...
********************************************************************/
public static function actualizar($id,$campos,$imagen,$galeria1,$galeria2,$galeria3,$galeria4){
	try {
		$db=Db::getConnect();
		$campostraidos = $campos->getCampos();
		extract($campostraidos);

		$update=$db->prepare('UPDATE nosotros SET 
								titulo=:titulo,
								texto1=:texto1,
								texto2=:texto2,
								imagen=:imagen,
		...
		$update->bindValue('texto2',$texto2);
		$update->bindValue('imagen',$imagen);
		$update->bindValue('galeria1',$galeria1);		
		$update->bindValue('galeria2',$galeria2);
		$update->bindValue('galeria3',$galeria3);
		$update->bindValue('galeria4',$galeria4);
		$update->bindValue('mision',$mision);
		$update->bindValue('vision',$vision);
		$update->bindValue('porque',$porque);
