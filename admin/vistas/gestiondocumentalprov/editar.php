...
foreach ($campos as $campo){
    ...
    if ($modulo_id_modulo==1) {
    	$asociado=Gestiondocumentaleq::ObtenerNombrecuenta($cuenda_id_cuenta);
    }
   	elseif ($modulo_id_modulo==2) {
   		$asociado=Gestiondocumentaleq::ObtenerNombreEquipo($cuenda_id_cuenta);
   	}
   		elseif ($modulo_id_modulo==5) {
   		$asociado=Gestiondocumentalprov::ObtenerNombreProveedor($cuenda_id_cuenta);
   	}
}
?>
      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active"><a href="?controller=Gestiondocumentalprov&&action=todos&&id_modulo=<?php echo($modulo_gestion); ?>&&id=<?php echo($id_cuenta); ?>">Documentos</a></li>
          </ol>
        </div><!-- /.col -->
      </div><!-- /.row -->
    </div><!-- /.container-fluid -->
					...
					<div class="row">
						...
						<div class="col-md-12">
						  ...
						  <div class="card card-primary">
							<!-- /.card-header -->
							<!-- form start -->
							<form role="form" action="?controller=gestiondocumentalprov&&action=actualizar&&idreg=<?php echo $id; ?>&&id_modulo=<?php echo($modulo_gestion); ?>&&id=<?php echo($id_cuenta) ?>" method="POST" enctype="multipart/form-data">
								<?php  
											...
											<label for="sel1">Tipo Documento:<span>*</span></label>
														  <select class="form-control"  name="documento_id_documento" >
															<option selected="" value="<?php echo($documento_id_documento) ?>" selected><?php echo utf8_encode($nombredocumento) ?></option>
																<?php
																	$cajas = Gestiondocumentaleq::obtenerDocumentos($modulo_gestion);
									...
									<div class="form-group">
											<label for="sel1">Asociar documento a Cuenta:<span>*</span></label>
														  <select class="form-control"  name="cuenta_id_cuenta" >
																...
																<?php
																	$cajas = Gestiondocumentaleq::obtenerCuentas();
																	foreach($cajas as $caja){
									...
									<div class="form-group">
											<label for="sel1">Asociar documento a Equipo:<span>*</span></label>
														  <select class="form-control"  name="cuenta_id_cuenta" >
																  <option selected="" value="<?php echo($cuenda_id_cuenta) ?>" selected><?php echo utf8_encode($asociado) ?></option>
																<?php
																	$cajas = Gestiondocumentaleq::obtenerEquipos();
																	foreach($cajas as $caja){
																		$id = $caja['id_equipo'];
									...
									<div class="form-group">
											<label for="sel1">Asociar documento a Equipo:<span>*</span></label>
														  <select class="form-control"  name="cuenta_id_cuenta" >
																  <option selected="" value="<?php echo($cuenda_id_cuenta) ?>" selected><?php echo utf8_encode($asociado) ?></option>
																<?php
																	$cajas = Gestiondocumentalprov::obtenerEquipos();
																	foreach($cajas as $caja){
																		$id = $caja['id_proveedor'];
																		$nombre = $caja['nombre_proveedor'];
																?>
																<option value="<?php echo $id; ?>"><?php echo utf8_encode($nombre); ?></option>
																<?php }?>
														  </select>
