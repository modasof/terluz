...
include_once 'vistas/index/header-formdate.php';
 ...
 $CuentaSel=$_GET['id_cuenta'];
  $nombrecuenta = Cuentas::obtenerNombreCuenta($CuentaSel);
   $ingresoscuenta = Informes::SumaIngresosporcuenta($FechaStart,$FechaEnd,$CuentaSel);
   $egresoscuenta = Informes::SumaEgresosporcuenta($FechaStart,$FechaEnd,$CuentaSel);
 ?>
    ...
    <div class="container-fluid">
      ...
      <div class="card card-dark">
              <div class="card-header">
                <h3 class="card-title">Entradas   $<?php echo(number_format($ingresoscuenta,2)); ?> </h3>
         ...
         <form action="?controller=informes&&action=movimientoscuentas&&id_cuenta=<?php echo($CuentaSel) ?>" method="post" id="FormFechas" autocomplete="off">
         <div class="col-md-8">
...
?>
  ...
  console.log('New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')');
          ...
          <tbody>
            <?php
            if ($fechaform!="") {
              $res=Informes::ListaIngresos($FechaStart,$FechaEnd,$CuentaSel);
             ;
            }
            else
            {
            ...
            $totalingresos = Informes::SumaIngresosportipo($FechaStart,$FechaEnd,$CuentaSel,$ingreso_por);
            $porcentajeingresos=($totalingresos/$ingresoscuenta)*100;
            ...
            <tr>
              ...
              <td>
                <?php Echo utf8_encode("$".number_format($totalingresos));  ?>
              </td>
              <td>
      ...
      <div class="card card-dark">
              <div class="card-header">
                <h3 class="card-title">Egresos   $<?php echo(number_format($egresoscuenta,2)); ?> </h3>
...
?>
  ...
  console.log('New date range selected: ' + start.format('YYYY-MM-DD') + ' to ' + end.format('YYYY-MM-DD') + ' (predefined range: ' + label + ')');
          ...
          <tbody>
            <?php
            if ($fechaform!="") {
              $res=Informes::ListaEgresos($FechaStart,$FechaEnd,$CuentaSel);
             ;
            }
            else
            {
            ...
            $totalegresos = Informes::SumaEgresosportipo($FechaStart,$FechaEnd,$CuentaSel,$rubrocuenta);
            $porcentajeegresos=($totalegresos/$egresoscuenta)*100;
            
            ?>
            <tr>
              
              <td><a href="?controller=informes&&action=subrubrosegresos&&id_rubro=<?php echo($rubrocuenta) ?>&&id_cuenta=<?php echo($CuentaSel); ?>"><?php echo $nombrerubro; ?></a></td>
              <td>
