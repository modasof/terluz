...
<h3 class="box-title">Proyecto : <?php echo utf8_decode($nombre_obra); ?> <strong>Fecha Inicio : <?php echo ($fecha_inicio); ?> </strong></h3>
   ...
   <div class="clearfix">
    ...
    <div>
        ...
        - <a class="toggle-vis btn btn-info btn-sm" data-column="7">Cantidades (+)</a> 
        - <a class="toggle-vis btn btn-info btn-sm" data-column="8">Valor (+)</a> 
        - <a class="toggle-vis btn btn-info btn-sm" data-column="9">Cantidades (-)</a> 
        - <a class="toggle-vis btn btn-info btn-sm" data-column="10">Valor (-)</a> 
        - <a class="toggle-vis btn btn-info btn-sm" data-column="11">Cant. Final</a>
        - <a class="toggle-vis btn btn-info btn-sm" data-column="12">Total Final</a>
    </div>
...
foreach ($campos as $campo) {
    ...
    <tr class="info">
      ...
      <td>
        ...
        if ($porcentaje_cap>0) {
          echo("<strong class='text-success'>".round($porcentaje_cap,2)." % <i class='fa fa-arrow-circle-up'></i></strong>");
        }elseif($porcentaje_cap<0){
          echo("<strong  class='text-danger'>".round($porcentaje_cap,2)." % <i class='fa fa-arrow-circle-down'></i></strong>");
        }elseif ($porcentaje_cap==0) {
    ...
    <td style="font-weight: bold;">
         <?php echo ("$" . number_format($sqlmayores_cap,0)); ?>
    </td>
    <td><strong>Cantidades (-)</strong></td>
   <td style="font-weight: bold;">
         <?php echo ("$" . number_format($sqlmenores_cap,0)); ?>
    </td>
    <td></td>
    <td style="font-weight: bold;">
...
# ===============================================
    ...
    $totalcantidades=($cantidad+$sqlcantidadesmayores)-$sqlcantidadesmenores;
       ...
       $porcentaje_act = formulaporcentaje($totalcantidades,$cantidad)-100;
        ...
        if ($porcentaje_act>0) {
          echo("<strong class='text-success'>".round($porcentaje_act,2)." % <i class='fa fa-arrow-circle-up'></i></strong>");
        }elseif($porcentaje_act<0){
          echo("<strong  class='text-danger'>".round($porcentaje_act,2)." % <i class='fa fa-arrow-circle-down'></i></strong>");
        }elseif ($porcentaje_act==0) {
          echo("<strong  class='text-info'>".round($porcentaje_act,2)." % </strong>");
        }
...
<form action="?controller=obras&&action=guardarcantmayores&&id=<?php echo($getobra); ?>" method="post" autocomplete="off">
    ...
    <input type="hidden" name="capitulo_id_capitulo" value="<?php echo($capitulo_id_capitulo);?>">
        <input type="hidden" name="fecha_reporte" value="<?php echo($campo_diaactual) ?>">
        <input type="hidden" name="menores" value="0">
                  ...
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      ...
                      <div class="col-md-12 col-xs-12">
                          <label>Agregar Cantidades<span>*</span></label>
                          
                          <input type="number" step="any" name="mayores" placeholder="Cantidades Mayores" class="form-control" required value="">
...
<?php 
 ...
 ?>
      <?php 
      echo(number_format($sqlcantidadesmayores,2));
       ?>
       <a href="#" onclick="eliminar(<?php echo $id_actividad; ?>,<?php echo $getobra; ?>);" class="pull-right tooltip-primary text-danger" data-rel="tooltip" data-placement="top" title="" data-original-title="Eliminar">
                <i class="fa fa-trash bigger-110 "></i>
      </a>
    </td>
    <td>
      <?php 
      echo utf8_decode("$".number_format($valorcantidadesmayores,0)) ?>
    </td>


   <?php 
  if ($sqlcantidadesmenores>0) {
    echo("<td class='danger'>");
  }else{
     echo("<td>");
  }
   ?>
      <label><a data-toggle="modal" data-target="#modal-form-menores<?php echo utf8_decode($id_actividad); ?>" href="#" class="text-success"><i class="fa fa-minus-square"></i></a></label>

      <?php 
      # =================================================================================
      # =           Inicio de ventana modal para registrar cantidades Menores           =
      # =================================================================================
       ?>
  
    <div id="modal-form-menores<?php echo utf8_decode($id_actividad); ?>" class="modal" tabindex="-1">
               <!-- Inicio Modal -->
    <div>
...
<form action="?controller=obras&&action=guardarcantmayores&&id=<?php echo($getobra); ?>" method="post" autocomplete="off">
    ...
    <input type="hidden" name="capitulo_id_capitulo" value="<?php echo($capitulo_id_capitulo);?>">
        <input type="hidden" name="fecha_reporte" value="<?php echo($campo_diaactual) ?>">
        <input type="hidden" name="mayores" value="0">
        <input type="hidden" name="creado_por" value="<?php echo($IdSesion);?>">
        <input type="hidden" name="estado_cantidades" value="1">
        <input type="hidden" name="cantidades_publicado" value="1">
        <input type="hidden" name="marca_temporal" value="<?php echo($campo_marcatemporal) ?>">
                  ...
                  <div class="modal-dialog ">
                    <div class="modal-content">
                      ...
                      <div class="col-md-12 col-xs-12">
                          <label>Quitar Cantidades<span>*</span></label>
                          
                          <input type="number" step="any" name="menores" placeholder="Cantidades Menores" class="form-control" required value="">
                          <small>Decimales separados con punto</small>
                      </div>
                         <br><br>
 ...
 <!-- FINAL MODAL -->
<?php 
  # ======  End of Modal Cantidades Menores  =======
  
 ?>
      <?php 
      echo(number_format($sqlcantidadesmenores,2));
       ?>
     ...
     <td>
      ...
      echo utf8_decode("$".number_format($valorcantidadesmenores,0)) ?>
    </td>
    <td class="info">
      <?php 
     
       echo (number_format($totalcantidades,2)); 
       ?>
    </td>
    <td>
      <?php 
      $valorfinal=$totalcantidades*$valor_unitario;
