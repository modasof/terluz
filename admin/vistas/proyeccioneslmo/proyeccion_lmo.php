...
<div class="box-body">
   <form role="form" action="?controller=proyeccioneslmo&&action=guardar&&id=<?php echo ($getobra); ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
<?php  
...
<dd>
  <div class="col-md-12">
    ...
    <div class="col-md-12">
      <strong>Listado de Mano de Obra</strong>
      <select name="lmo_id_lmo" class="form-control mi-selector" >
        <option value="">Seleccionar... </option>
        <?php
$cajas = Listamo::obtenerLista();
foreach ($cajas as $caja) {
    $id_lmo           = $caja['id_lmo'];
    $nombre_lmo       = $caja['nombre_lmo'];
    $unidad_id_unidad = $caja['unidad_id_unidad'];
    $nomunidad        = Unidadesmed::obtenerNombre($unidad_id_unidad);

    ?>

                <option value="<?php echo $id_lmo; ?>"><?php echo utf8_decode($nombre_lmo . " [" . $nomunidad . "]"); ?></option>
    ...
    <input type="number" step="any" name="cantidad_lmo" placeholder="Cantidades Proyectadas" class="form-control" required value="">
    ...
    <div class="col-md-12">
            <div class="form-group">
                <label>Valor Unitario: <span>*</span></label>
                    <input type="text" name="valor_unitario_lmo" placeholder="Valor del gasto" class="form-control" id="demo1">
            </div>
    </div>
...
<div style="background-color: white;" class="col-md-12">
  ...
  <div class="box-header with-border">
    <h1>PROYECCIÓN DE MANO DE OBRA </h1>
<i class="fa fa-info-circle"> </i>
<h3 class="box-title"><?php echo utf8_decode($nombre_obra); ?> <strong>Fecha Inicio : <?php echo ($fecha_inicio); ?> </strong></h3>
</div>
...
foreach ($campos as $campo) {
    ...
    $nombrecampo = Listamo::obtenerNombre($lmo_id_lmo);
    $unidad=Listamo::obtenerUnidadmedida($lmo_id_lmo);
...
?>
 ...
 <tr>
          <td>
            ...
            <br>
    <a href="#" onclick="eliminarequipo(<?php echo $getobra; ?>,<?php echo $lmo_id_lmo; ?>);" class="pull-right tooltip-primary text-danger" data-rel="tooltip" data-placement="top" title="" data-original-title="Eliminar">
                <i class="fa fa-trash bigger-110 "> Eliminar MO</i>
    </a>
        </td>
        <td>
...
$rangos       = Obras::rangosporobra($getobra);
 ...
 <td>
     <?php
        ...
        ?>
         <a href="#" onclick="eliminar(<?php echo $id_rango_act; ?>,<?php echo $getobra; ?>,<?php echo $lmo_id_lmo; ?>);" class="pull-right tooltip-primary text-danger" data-rel="tooltip" data-placement="top" title="" data-original-title="Eliminar">
...
function eliminar(rango,obra,equipo){
   ...
   if (eliminar)
     window.location.href="?controller=proyeccioneslmo&&action=eliminarango&&id="+obra+"&&idrango="+rango+"&&equipolme="+equipo;
else
...
function eliminarequipo(obra,equipo){
   eliminar=confirm("¿Deseas eliminar este registro?");
   if (eliminar)
     window.location.href="?controller=proyeccioneslmo&&action=eliminaequipo&&id="+obra+"&&equipolme="+equipo;
else
  //Y aquí pon cualquier cosa que quieras que salga si le diste al boton de cancelar
    alert('No se ha podido eliminar el registro...')
}
