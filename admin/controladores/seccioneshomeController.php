...
/*************************************************************/
	...
	function todos() { 
		$campos=Testimonios::obtenerPagina();;
		require_once 'vistas/seccioneshome/todos.php';
	}
...
/*************************************************************/
	function nuevo() {
		require_once 'vistas/seccioneshome/nuevo.php';
	}
...
/*************************************************************/
	function editar() {
		$id = $_GET['id'];
		$campos = Testimonios::obtenerPaginaPor($id);
		require_once 'vistas/seccioneshome/editar.php';
	}
...
/*************************************************************/
	function eliminar() {
		$id = $_GET['id'];
		$res = Testimonios::eliminarPor($id);
		if ($res){
...
function guardarTestimonio() {
	...
	$campo = new Testimonios('',$nuevoarreglo);
	$res = Testimonios::guardarTestimonio($campo,$ruta_imagen);
	if ($res){
...
function actualizarTestimonio(){
	if (empty($_FILES['imagen']['name'])){
		...
		$ruta_imagen=$this->subir_fichero('../pagina/images/seccioneshome','imagen');	
	}
	$id = $_GET['id'];
	$variable = $_POST;
	$nuevoarreglo = array();
	...
	$datosguardar = new Testimonios($id,$nuevoarreglo);
	$res = Testimonios::actualizarTestimonio($id,$datosguardar,$ruta_imagen);
	if ($res){
