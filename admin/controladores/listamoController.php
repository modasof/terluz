...
/*************************************************************/
	...
	function todos() {
		$campos=Listamo::obtenerPagina();;
		require_once 'vistas/listamo/todos.php';
	}
...
/*************************************************************/
	function editar() {
		$id = $_GET['id'];
		$campos = Listamo::obtenerPaginaPor($id);
		require_once 'vistas/listamo/editar.php';
	}
...
function guardar() {
	...
	$campo = new Listamo('',$nuevoarreglo);

	$nombre_lmo=$_POST['nombre_lmo'];
	$unidad = $_POST['unidad_id_unidad'];

	$validacion = Listamo::validacionduplicado($nombre_lmo,$unidad);

	if ($validacion>0) {
		echo "<script>jQuery(function(){Swal.fire(\"¡Registro ya Existe!\", \"Verifique la información\", \"error\");});</script>";
	}
else{
	$res = Listamo::guardar($campo);
	if ($res){
...
function actualizar(){
	...
	$datosguardar = new Listamo($id,$nuevoarreglo);
	$res = Listamo::actualizar($id,$datosguardar);
	if ($res){
